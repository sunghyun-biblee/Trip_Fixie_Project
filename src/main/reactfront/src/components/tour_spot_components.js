...
const TButton = styled.button`
  ...
  cursor: pointer;
`;
export const TourSpotList = ({ tourList, isMainLoading, setSaveTourList, setIsSlideMode, saveTourList, handleDeleteList, }) => {
...
export const FestivalSpotList = ({ festivalList, isMainLoading, setSaveTourList, setIsSlideMode, saveTourList, handleDeleteList, }) => {
  const scrollBoxRef = useRef();
  const [list, setList] = useState(0);
  const [isLoading, setIsLoading] = useState(false);
  const [currtourList, setCurrTourList] = useState();
