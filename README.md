...
### [결과]
 ...
 - 클라이언트 측에서 초기 데이터 요청 시 모든 데이터를 요청하여 이중 배열을 통해 데이터를 나누어 저장
 - 리스트 박스 하단에 닿을 시 스크롤 이벤트 발생 시켜 인덱스를 증가
 - 증가된 인덱스를 참조하여 데이터를 출력 하도록 변경한 결과, 일일 API 호출 수를 약 85% 감소
...
Firebase의 auth.currentUser를 직접 사용해 초기 로그인 여부에 따라 UI를 설정하려 했으나, 로그인 이후 새로고침 시 자동으로 로그아웃되는 문제 발생
